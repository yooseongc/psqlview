...
const ROWS: &[(&str, &str)] = &[
    ...
    (":N", "goto line N"),
    (":s/pat/repl/[g]", "substitute on current line (g = all)"),
    (":%s/pat/repl/[g]", "substitute across the whole buffer"),
    (":w [path]", "save current tab (path arg overrides existing)"),
    (":e <path>", "open path into the active tab"),
    (
    ...
    ("x / s", "delete char / change char (Visual: delete/change)"),
    ("p / P", "paste from register after / before cursor"),
    (
        "iw aw / iW aW",
        "small word / WORD (whitespace-bounded; SQL: schema.table)",
    ),
    ("i\" a\" / i( a(", "quoted string / parens (inner / around)"),
    ("", ""),
    ("Editor", ""),
