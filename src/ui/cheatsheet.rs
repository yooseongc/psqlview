...
const ROWS: &[(&str, &str)] = &[
    ...
    ("", ""),
    ("Editor — Normal / Visual search", ""),
    ("/pat<Enter>", "search forward  (Visual: extends selection)"),
    (
        "?pat<Enter>",
        "search backward  (Visual: extends selection)",
    ),
    ("n  /  N", "repeat last search  (Visual: extends selection)"),
    ("", ""),
    ...
    (
        ":s/pat/repl/[gc]",
        "substitute current line  (g = all  ·  c = confirm)",
    ),
    (":%s/pat/repl/[gc]", "substitute whole buffer"),
    (":w  [path]", "save  (Tab + Up/Down browse cwd)"),
    (":e <path>", "open path  (Tab + Up/Down browse cwd)"),
    (
    ...
    (
        "Ctrl+O  /  Ctrl+S",
        "open / save file  (Up/Down select \u{00b7} Tab commit)",
    ),
    ...
    ("e", "edit cell  (tree-preview results, single-PK tables)"),
    ("R", "re-run last query  (or refresh DDL view)"),
    ("", ""),
    ("Substitute confirm (`:s/.../c`)", ""),
    ("y", "replace and advance"),
    ("n", "skip and advance"),
    ("a", "replace all remaining and close"),
    ("q  /  Esc", "quit  (keeps replacements made so far)"),
    ("", ""),
    ("Cell edit + UPDATE confirm", ""),
    (
        "type \u{2192} Enter",
        "build UPDATE  (Ctrl+U clears \u{2192} NULL)",
    ),
    ("y  /  n", "execute UPDATE  /  cancel"),
    ("Esc", "abort cell edit"),
    ("", ""),
    ("Status bar transaction badge", ""),
    ("[TX] yellow", "inside an explicit transaction"),
    ("[TX!] red", "transaction in error  (ROLLBACK to recover)"),
    ("", ""),
    ("Row detail (modal)", ""),
    ("Up Down  /  j k", "scroll"),
    ("PageUp  /  PageDown", "page"),
